...
/*
void Agencia::adicionaItinerario(Itinerario itn){
	itinerarios.push_back(itn);
}*/

/*
bool Agencia::apagaItinerario(string origem, string destino){
	for(int i=0; i<itinerarios.size(); i++){
		if(itinerarios[i].getOrigem() == origem
		&& itinerarios[i].getDestino() == destino){
			itinerarios.erase(itinerarios.begin()+i);
			return true;
		}
	}
			...
			if (tipo == "Particular") {
				...
				for (int i = 0; i < numViagens; i++) {
					...
					if (alojamento != "NULO") {
						for (size_t i = 0; i < alojamentos.size(); i++) {
							if (alojamentos[i].getNome() == alojamento && alojamentos[i].getLocal() == destino)
								precoTrocos += alojamentos[i].getPreco();
						}
					}
				...
				for (int i = 0; i < numViagens; i++) {
					...
					if (precoTotal != precoTrocos - precoTrocos * c1->calcDesconto()) {
						...
						if (alojamento == "NULO") {
							venderViagem(oss.str(), i1.getOrigem(), i1.getDestino(), 0);
						} else {
